...
### ⚠️ Post-Installazione (OBBLIGATORIO)

Dopo l'installazione automatica, **devi eseguire le migrazioni del database**:

```bash
cd /opt/agrisecure/backend

# 1. Crea le migrazioni per i modelli
sudo -u agrisecure venv/bin/python manage.py makemigrations nodes sensors security core

# 2. Applica le migrazioni al database
sudo -u agrisecure venv/bin/python manage.py migrate

# 3. Raccogli i file statici
sudo -u agrisecure venv/bin/python manage.py collectstatic --noinput

# 4. Crea superuser per accedere al sistema
...
# Migrazioni database
sudo -u agrisecure backend/venv/bin/python backend/manage.py makemigrations
...
Le tabelle del database non esistono. Esegui le migrazioni:
```bash
cd /opt/agrisecure/backend
sudo -u agrisecure venv/bin/python manage.py makemigrations nodes sensors security core
sudo -u agrisecure venv/bin/python manage.py migrate
