#!/usr/bin/env python3
"""
AgriSecure IoT Simulator v3
===========================
Fix: loop MQTT corretto con threading
"""
...
class Simulator:
    ...
    def _on_disconnect(self, client, userdata, rc):
        self.connected = False
        if rc != 0:
            print(f"⚠️ Disconnesso: {rc}")
    
    def _on_publish(self, client, userdata, mid):
        # Conferma pubblicazione (debug)
        pass
    ...
    def start(self):
        ...
        # Invia subito un heartbeat per ogni nodo
        print("📤 Invio heartbeat iniziale...")
        for node in self.nodes.values():
            self._send_heartbeat(node)
...
def main():
    parser = argparse.ArgumentParser(description="AgriSecure Simulator v3")
    parser.add_argument("--auto", action="store_true")
    parser.add_argument("--duration", type=int, default=0)
    parser.add_argument("--scenario", default="normal")
    parser.add_argument("--broker", default="localhost")
    parser.add_argument("--port", type=int, default=1883)
    parser.add_argument("--username", default="agrisecure")
    parser.add_argument("--password", default="mqtt_secure_password")
    
    args = parser.parse_args()
    
    print("""
    ╔═══════════════════════════════════════════════════════╗
    ║         🌿 AgriSecure Simulator v3 🌿                 ║
    ╚═══════════════════════════════════════════════════════╝
    """)
